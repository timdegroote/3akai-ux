...
<div id="oa-publicationform-form-template"><!--
    ...
    {macro inputLabel(name, label, required, helpText, helpModalId, helpModalText)}
        <label for="oa-publicationform-input-${name}" class="control-label{if required} oa-publicationform-required{/if}" {if helpText}data-toggle="popover" data-trigger="hover" data-content="${helpText}"{/if}>${label}{if helpText} <i class="icon icon-question-sign"></i>{/if}</label>
        {if helpModalId}
